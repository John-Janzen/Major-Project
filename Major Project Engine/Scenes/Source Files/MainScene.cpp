...
bool MainScene::LoadWalls(SceneManager & sm) 
{
	...
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(w_wall ->GetID(), btQuaternion(0.f, 0.f, 1.57f), btVector3(btScalar(50.f), btScalar(25.f), btScalar(0.f))));
	sm.AddComponent(SceneManager::PHYSICS, new WallPhysicsComponent(w_wall->GetID()));
	sm.AddComponent(SceneManager::RENDER, new FloorRenderComponent(w_wall->GetID()));

	Entity * & e_wall = sm.CreateEntity("Wall_east");
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(e_wall->GetID(), btQuaternion(0.f, 0.f, -1.57f), btVector3(btScalar(-50.f), btScalar(25.f), btScalar(0.f))));
	sm.AddComponent(SceneManager::PHYSICS, new WallPhysicsComponent(e_wall->GetID()));
	sm.AddComponent(SceneManager::RENDER, new FloorRenderComponent(e_wall->GetID()));

	Entity * & n_wall = sm.CreateEntity("Wall_north");
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(n_wall->GetID(), btQuaternion(0.f, -1.57f, 0.f), btVector3(btScalar(0.f), btScalar(25.f), btScalar(50.f))));
...
bool MainScene::LoadFloor(SceneManager & sm)
{
	...
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(floor->GetID(), btVector3(btScalar(0.f), btScalar(0.f), btScalar(0.f))));
...
bool MainScene::LoadPlayer(SceneManager & sm)
{
	...
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(player->GetID(), btVector3(btScalar(0.f), btScalar(2.f), btScalar(30.f))));
	...
	sm.AddComponent(SceneManager::CONTROLLER, new PlayerControllerComponent(player->GetID()));
	return true;
}
...
bool MainScene::LoadMultiObject(SceneManager & sm, const float x, const float y, const float z)
{
	...
	sm.AddComponent(SceneManager::TRANSFORM, new Transform(m_object->GetID(), btQuaternion(btScalar(0.f), btScalar(0.f), btScalar(1.f)), btVector3(x, y + 1 + ((NUM_MULTI_OBJECTS - 1) * 1.5f), z)));
